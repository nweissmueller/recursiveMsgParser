 ...
 * through the original string, and takes in the combination
 * output vector by value to feed solutions back
 * to the caller function paseMessage().
 */
Vector<string> parseMessageHelper2(Lexicon& lex, string message, int n,
    ...
    while (count <= message.length()) {
        // recursive case: parse substring from 0 to i
        string word = message.substr(0, count);
        if (lex.contains(word)) {
            // recursive case remove valid word result from left of string and recurse
            combinations = parseMessageHelper2(lex, message.substr(count, n-count), n-count,
 ...
 */
Vector<string> parseMessage2(Lexicon& lex, string message) {
    // initialize variables
    Vector<string> combinations;
    int n = message.size();

    // recursively parse string into valid words
    combinations = parseMessageHelper2(lex, message, n, "", combinations);
...
PROVIDED_TEST("Fringe cases: empty string, gobly gook, not all valid words") {
    ...
    // parse the message
    Vector<string> emptyRes = parseMessage2(lex, empty);
    Vector<string> gookRes = parseMessage2(lex, gook);
    Vector<string> notValidRes = parseMessage2(lex, notAllValidWords);
...
PROVIDED_TEST("Test message: appleorchard") {
    ...
    // parse the message
    Vector<string> combinations = parseMessage2(lex, message);
